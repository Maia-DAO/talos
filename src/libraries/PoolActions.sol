...
library PoolActions {
    ...
    function rerange(
        ...
        RerangeParams memory rerangeParams
    )
        internal
        returns (int24 tickLower, int24 tickUpper, uint256 amount0, uint256 amount1, uint256 tokenId, uint128 liquidity)
    {
        ...
        (amount0, amount1, tickLower, tickUpper) = rerangeParams.getTicksAndAmounts(
            actionParams.pool,
            actionParams.token0,
            actionParams.token1,
            baseThreshold,
        ...
        (tokenId, liquidity, amount0, amount1) = nonfungiblePositionManager.mint(
            INonfungiblePositionManager.MintParams({
                ...
                fee: rerangeParams.poolFee,
                tickLower: tickLower,
                tickUpper: tickUpper,
                amount0Desired: amount0,
                amount1Desired: amount1,
                amount0Min: rerangeParams.amount0Min,
                amount1Min: rerangeParams.amount1Min,
                recipient: address(this),
                deadline: rerangeParams.deadline
            })
        );
    }
