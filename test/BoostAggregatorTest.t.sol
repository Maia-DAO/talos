...
contract BoostAggregatorTest is TalosTestor {
    ...
    function test_fail_addAllowlistedAddress(address user) public {
        if (user != address(this)) vm.expectRevert(Ownable.Unauthorized.selector);
        vm.prank(user);
        boostAggregator.addAllowlistedAddress(user);
    }
    ...
    function test_fail_setProtocolFee(uint256 fee) public {
        ...
        vm.expectRevert(IBoostAggregator.FeeTooHigh.selector);
        boostAggregator.setProtocolFee(fee);
    }

    // Set protocol fee fail test, not called by owner
    function test_fail_setProtocolFee_notOwner(uint256 fee) public {
        if (msg.sender != address(this)) vm.expectRevert(Ownable.Unauthorized.selector);
        vm.prank(msg.sender);
        boostAggregator.setProtocolFee(fee);
    }
